...
public sealed class NetFlowMetricAggregator : BackgroundService
{
    ...
    // Double-buffering: два экземпляра переиспользуются по кругу через Clear(),
    // вместо `new MetricBucket()` (4 ConcurrentDictionary) каждую секунду.
    private MetricBucket _currentBucket = new();
    private MetricBucket _spareBucket   = new();
    ...
    private async Task FlushLoopAsync(CancellationToken ct)
    {
        ...
        while (await timer.WaitForNextTickAsync(ct).ConfigureAwait(false))
        {
            ...
            // 1. Атомарно подменяем бакет на заранее подготовленный "запасной".
            //    AccumulateLoop при следующем Volatile.Read увидит новый бакет.
            var retired = Interlocked.Exchange(ref _currentBucket, _spareBucket);

            // 2. Извлекаем снимок с top-10 min-heap (stackalloc, без LINQ).
            var snapshot = retired.TakeSnapshot(now);

            // Очищаем "отставленный" бакет и делаем его запасным для следующего тика —
            // ноль аллокаций ConcurrentDictionary на горячем пути FlushLoop.
            retired.Clear();
            _spareBucket = retired;
