...
// Именованный HttpClient с переиспользуемым SocketsHttpHandler/пулом соединений —
// ClickHouseConnectionFactory создаёт ClickHouseConnection через IHttpClientFactory,
// чтобы не плодить новый handler/сокет на каждый flush (1 раз/сек).
// AutomaticDecompression обязателен: ClickHouse по умолчанию возвращает gzip-сжатые
// ответы, а SocketsHttpHandler без этой настройки их не распаковывает — драйвер
// получает сжатые байты и падает с InvalidOperationException при чтении заголовков.
builder.Services
...
app.Map(wsPath, async context =>
{
    ...
    try
    {
        ...
        while (ws.State == System.Net.WebSockets.WebSocketState.Open)
        {
            ...
            if (result.MessageType == System.Net.WebSockets.WebSocketMessageType.Close
                && ws.State == System.Net.WebSockets.WebSocketState.CloseReceived)
            {
                await ws.CloseAsync(
    ...
    {
        // Сервер останавливается (Ctrl+C) или клиент уже отключился — штатное завершение.
    }
    catch (System.Net.WebSockets.WebSocketException)
    {
        // Соединение прервано клиентом/сетью (Aborted) — штатное завершение.
    }
    finally
    {
        hub.Unregister(ws);
